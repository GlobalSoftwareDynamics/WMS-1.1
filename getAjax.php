...
if (!empty($_POST['getprecioprom2'])) {
    ...
    $nombreProducto = explode("_",$_POST['getprecioprom2']);
    $id=mysqli_query($link,"SELECT * FROM Producto WHERE nombreCorto = '{$nombreProducto[0]}'");
    while ($fila=mysqli_fetch_array($id)){
        $idProducto = $fila['idProducto'];
        $array[$aux]=array($fila['costoEstimado'],'NA');
    }
    $catalogos=mysqli_query($link,"SELECT * FROM Catalogo WHERE fechaInicio <= '{$date}' AND fechaFin >= '{$date}'");
    while ($row=mysqli_fetch_array($catalogos)){
        $campanaactual=$row['idCampana'];
        $idcatalogoactual=$row['idCatalogo'];
    }
    ...
    if($numrows>0){
        while ($row1=mysqli_fetch_array($catalogoscampana)){
            $aux++;
            if(!empty($row1['promocion'])){
                $array[$aux]=array($row1['precio'],$row1['promocion']);
            }else{
                $array[$aux]=array($row1['precio'],"NA");
            }
        }
    }else{
        $aux++;
    ...
    echo "
        <select class='form-control' name='precio' onchange='montorestante({$_POST['totalPrestado']},this.value);fechavenc({$_POST['totalPrestado']},this.value)'>
            <option disabled>Opciones</option>";
    for ($i=0;$i<4;$i++){
        if($i===0){
            echo "
            ...
            echo "
                <option value='{$array[$i][0]}'>Catálogo Anterior: S/.{$array[$i][0]} ({$array[$i][1]})</option>
            ";
        }elseif ($i===2){
            echo "
                <option value='{$array[$i][0]}'>Catálogo Actual: S/.{$array[$i][0]} ({$array[$i][1]})</option>
            ";
        }elseif ($i===3){
            echo "
                <option value='{$array[$i][0]}'>Catálogo Siguiente: S/.{$array[$i][0]} ({$array[$i][1]})</option>
            ";
        }
    }
    echo "
